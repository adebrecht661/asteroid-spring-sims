...
void print_heat(struct reb_simulation *const r, int npert, char *filename,
	...
	double theta = atan2(y1 - CoM.getY(), x1 - CoM.getX());
	double ct = cos(theta);
	double st = sin(-theta);  // note sign!
	fprintf(fpo, "#%.3f %.3f %.3f %.3f %.3f %.3f %.3f \n", x1, y1, z1, CoM.getX(),
			CoM.getY(), CoM.getZ(), theta);
	...
	for (int i = 0; i < NS; i++) {
		...
		double xrot = x_mid.getX() * ct - x_mid.getY() * st;
		double yrot = x_mid.getX() * st + x_mid.getY() * ct;
		...
		fprintf(fpo, "%d %.3f %.3f %.3f %.5e %.3f %.3f\n", i, x_mid.getX(), x_mid.getY(), x_mid.getZ(),
				power * powerfac, xrot, yrot);
		// put some heat on nodes as well as center of spring
		if (powerfac < 1.0) {
			int ii = springs[i].i;
			int jj = springs[i].j;
			double xpi = particles[ii].x - CoM.getX();
			double ypi = particles[ii].y - CoM.getY();
			double zpi = particles[ii].z - CoM.getZ();
			double xpj = particles[jj].x - CoM.getX();
			double ypj = particles[jj].y - CoM.getY();
			double zpj = particles[jj].z - CoM.getZ();
			fprintf(fpo, "%d %.3f %.3f %.3f %.5e %.3f %.3f\n", i, xpi, ypi, zpi,
...
void hfilename(struct reb_simulation *const r, char *root, double tp,
		char *fname) {
	int xd = (int) (r->t / tp);
	char junks[20];
	sprintf(junks, "%d", xd);
...
void nfilename(struct reb_simulation *const r, char *root, double tp,
		char *fname) {
	int xd = (int) (r->t / tp);
	char junks[20];
	sprintf(junks, "%d", xd);
...
void print_node(struct reb_simulation *const r, int npert, struct node *nodevec,
	...
	for (int i = il; i < ih; i++) {
		...
		fprintf(fpo, "%10.6e %10.6f %3d %10.6f ", nodevec[i].temp,
				nodevec[i].cv, nodevec[i].surf, m);
...
double *newTvec;
void transport_heat(struct reb_simulation *const r, int npert,
		struct node *nodevec, double dt) {
	static int first = 0;
	if (first == 0) {
		first = 1;
...
void heat_nodes_tidal(struct reb_simulation *const r, int npert,
		struct node *nodevec, double dt) {
	for (int k = 0; k < NS; k++) {
...
double Kthermal_mesh(struct reb_simulation *const r, int il, int ih,
		double rmin, double rmax) {
	double sum = 0.0;
	Vector CoM = compute_com(r, il, ih); // center of mass coords for particles in range
	for (int i = 0; i < NS; i++) {
		Vector x_mid = spr_mid(r, springs[i], CoM);

		double rc = x_mid.len(); // center of spring
		if ((rc < rmax) && (rc > rmin)) {
...
void adjust_spring_temp_lin(struct reb_simulation *r, struct node *nodevec,
		struct spring mush, double dksdT, double dgammadT, double dkheatdT) {
	for (int k = 0; k < NS; k++) {
		int ii = springs[k].i;
		int jj = springs[k].j;
...
void adjust_spring_temp_ts(struct reb_simulation *r, struct node *nodevec,
		struct spring mush_hot, struct spring mush_cold, double Ttrans) {
	for (int k = 0; k < NS; k++) {
		int ii = springs[k].i;
		int jj = springs[k].j;
		double Ti = nodevec[ii].temp;
...
void adjust_nodes_cp(struct reb_simulation *const r, int npert,
		struct node *nodevec, double cpnew, double Ttrans, int above) {
	int il = 0;
	int ih = r->N - npert;
	for (int i = il; i < ih; i++) {
