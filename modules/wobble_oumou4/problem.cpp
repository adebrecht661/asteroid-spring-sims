...
// Lattice selectors
const int RAND = 0, HCP = 1, CUBIC = 2;
...
int main(int argc, char *argv[]) {
	...
	if (!(cfg.lookupValue("fileroot", fileroot) && cfg.lookupValue("dt", dt)
			...
			&& cfg.lookupValue("surface_type", lattice_type)
	...
	const double boxsize = 3.8 * r_ball;
	...
	default: {
		throw "No lattice of requested type available. Exiting.";
	}
	}
...
void heartbeat(reb_simulation *const n_body_sim) {
	...
	// Recenter simulation
	subtract_com(n_body_sim, 0, n_body_sim->N);
